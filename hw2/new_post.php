    ...
    <div class="new-post">
      <form action="handle_new_post.php" method="post">
        ...
        <?php
            if (!empty($_GET)){
              if ($_GET['errorNo'] === '1') {
                echo "<h3 class='errorMessage'>標題或內容未填寫，請再輸入一次。</h3>";
              } else if ($_GET['errorNo'] === '2') {
                echo "<h3 class='errorMessage'>資料庫出錯，請重新輸入。</h3>";
              }
            }
        ?>
        <div class="new-post-title">
          <input class="new-post-title-input" type="text" name="title" id="" placeholder="請輸入文章標題">
        </div>
        <div class="new-post-content">
          <textarea class="new-post-content-input" name="content"rows="18"></textarea>
        </div>
